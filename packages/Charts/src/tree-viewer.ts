...
export class TreeViewer extends EChartViewer {
  ...
  constructor() {
    ...
    this.symbol = this.string('symbol', 'emptyCircle',
      {choices: ['circle', 'emptyCircle', 'rect', 'roundRect', 'triangle', 'diamond', 'pin', 'arrow', 'none']});
  ...
  parseNwk(s) {
    return {name: 'A', children: [
      {name: 'B', value: 123},
      {name: 'C', children: [{name: 'D', value: 456}, {name: 'E', value: 789}]},
    ]};
  }

  updateSeriesData() {
    this.newickCol = (this.dataFrame!.columns as DG.ColumnList).bySemType('newick');
    if (this.newickCol)
      this.setForestOpts();
    else
  ...
  onPropertyChanged(p: DG.Property, render = true) {
    ...
    for (const property of properties) {
      for (const chart of this.option.series!) {
        if (isForest && property.name === 'orient')
          continue;
        chart[property.name] = property.get(this);
      }
    }
